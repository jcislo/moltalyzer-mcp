...
export function register(server: McpServer, fetchFn: typeof fetch) {
  server.registerTool("get_pulse_narratives", {
    description:
      "Get all active Pulse narratives with lifecycle stage, source count, and momentum. Narratives track specific stories gaining traction across 2+ sources (Reddit, HN, GitHub, HuggingFace). Costs $0.01 USDC via x402.",
  }, async () => {
  ...
  server.registerTool("get_pulse_narrative_detail", {
    description:
      "Get detailed view of a specific Pulse narrative — full content items, source breakdown, and stage history. Use get_pulse_narratives first to find narrative IDs. Costs $0.01 USDC via x402.",
    inputSchema: {
